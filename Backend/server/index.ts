...
function broadcastPlayerMovement(room: Room, excludedSocketId: string, data: any) {
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room', (data: { username: string; room: 'office' | 'conference' }) => {
  ...
  socket.on('player-movement', (data: { x: number; y: number; direction: string }) => {
    ...
    playerState.direction = data.direction as 'up' | 'down' | 'left' | 'right';
  ...
  socket.on('check-proximity', (data: { x: number; y: number }) => {
    let nearbyPlayers: { id: string; username: string; x: number; y: number }[] = [];
