...
const isValidAvatarProfile = (profile) => {
    if (!profile || typeof profile !== 'object')
        return false;
    return (VALID_AVATAR_OPTIONS.body.includes(profile.body) &&
...
const makeToken = (user) => jwt.sign({ id: user.id, username: user.username }, JWT_SECRET, { expiresIn: '24h' });
const makeOAuthRedirect = (token) => {
    const url = new URL(CLIENT_URL);
    url.searchParams.set('oauth', 'success');
    url.searchParams.set('token', token);
    return url.toString();
};
const makeOAuthErrorRedirect = (error) => {
    const url = new URL(CLIENT_URL);
    url.searchParams.set('oauth', 'error');
    url.searchParams.set('message', error);
    return url.toString();
};
const toUsernameBase = (value) => value.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 16) || 'player';
...
async function findOrCreateMicrosoftUser(microsoftId, email, name) {
...
router.get('/oauth/google/callback', async (req, res) => {
    try {
        ...
        if (!code)
            return res.redirect(makeOAuthErrorRedirect('google_auth_failed'));
        ...
        if (!tokenJson.id_token)
            return res.redirect(makeOAuthErrorRedirect('google_id_token_missing'));
        ...
        if (!payload?.sub)
            return res.redirect(makeOAuthErrorRedirect('google_payload_invalid'));
        const user = await findOrCreateGoogleUser(payload.sub, payload.email || undefined, payload.name || undefined);
        if (!user)
            return res.redirect(makeOAuthErrorRedirect('user_creation_failed'));
...
router.get('/oauth/microsoft/callback', async (req, res) => {
    try {
        const { code } = req.query;
        if (!code)
            return res.redirect(makeOAuthErrorRedirect('microsoft_auth_failed'));
        ...
        const tokenJson = await tokenRes.json();
        if (!tokenJson.id_token)
            return res.redirect(makeOAuthErrorRedirect('microsoft_id_token_missing'));
